...
export function InventoryManagement({ searchQuery = '' }: { searchQuery?: string }) {
  ...
  const inventoryWithStatus = inventory.map(item => ({ ...item, calculatedStatus: getStatus(item) }));
  ...
  return (
    <div className="p-8 space-y-6">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="font-['Manrope'] text-gray-600">Monitor stock levels across all sellers</p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} onClick={() => setFilter('low')} className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-2xl p-6 border border-amber-200 cursor-pointer hover:shadow-md transition-shadow">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-12 h-12 bg-amber-500 rounded-xl flex items-center justify-center"><TrendingDown className="w-6 h-6 text-white" /></div>
            <div><p className="font-['Manrope'] text-xs text-amber-700">Low Stock</p><p className="font-['Crimson_Pro'] text-3xl font-bold text-amber-900">{stats.low}</p></div>
          </div>
          <button className={`text-xs font-['Manrope'] font-semibold ${filter === 'low' ? 'text-amber-800' : 'text-amber-600'}`}>View all →</button>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} onClick={() => setFilter('critical')} className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-2xl p-6 border border-orange-200 cursor-pointer hover:shadow-md transition-shadow">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-12 h-12 bg-orange-500 rounded-xl flex items-center justify-center"><AlertTriangle className="w-6 h-6 text-white" /></div>
            <div><p className="font-['Manrope'] text-xs text-orange-700">Critical</p><p className="font-['Crimson_Pro'] text-3xl font-bold text-orange-900">{stats.critical}</p></div>
          </div>
          <button className={`text-xs font-['Manrope'] font-semibold ${filter === 'critical' ? 'text-orange-800' : 'text-orange-600'}`}>View all →</button>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} onClick={() => setFilter('out')} className="bg-gradient-to-br from-red-50 to-red-100 rounded-2xl p-6 border border-red-200 cursor-pointer hover:shadow-md transition-shadow">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-12 h-12 bg-red-500 rounded-xl flex items-center justify-center"><Package className="w-6 h-6 text-white" /></div>
            <div><p className="font-['Manrope'] text-xs text-red-700">Out of Stock</p><p className="font-['Crimson_Pro'] text-3xl font-bold text-red-900">{stats.out}</p></div>
          </div>
          <button className={`text-xs font-['Manrope'] font-semibold ${filter === 'out' ? 'text-red-800' : 'text-red-600'}`}>View all →</button>
        </motion.div>
      </div>

      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white rounded-3xl p-6 shadow-lg border border-gray-100">
          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredInventory.map((item, index) => (
                <motion.tr key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + index * 0.05 }} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
