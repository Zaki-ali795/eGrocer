        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="relative">
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-['Manrope'] font-medium transition-all ${isFilterOpen ? 'bg-[var(--green-dark)] text-white' : 'bg-gray-50 text-gray-700 hover:bg-gray-100'}`}
            >
              ...
              <>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="absolute left-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-gray-100 z-20 p-4">
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto">
                    <button onClick={() => { setSelectedCategory('all'); setIsFilterOpen(false); }} className={`w-full text-left px-3 py-2 rounded-xl text-sm font-medium transition-colors ${selectedCategory === 'all' ? 'bg-[var(--primary)]/10 text-[var(--green-dark)]' : 'hover:bg-gray-50 text-gray-600'}`}>
                      All Products
                    </button>
                    {categoryList.map(cat => (
                      <button key={cat} onClick={() => { setSelectedCategory(cat); setIsFilterOpen(false); }} className={`w-full text-left px-3 py-2 rounded-xl text-sm font-medium transition-colors ${selectedCategory === cat ? 'bg-[var(--primary)]/10 text-[var(--green-dark)]' : 'hover:bg-gray-50 text-gray-600'}`}>
                        {cat}
                      </button>
                    ))}
                  </div>
                ...
                <motion.tr key={product.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + index * 0.05 }} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 bg-gradient-to-br from-[var(--primary)]/10 to-[var(--primary)]/20 rounded-xl flex items-center justify-center overflow-hidden">
                  ...
                  <td className="py-4 px-4">
                    <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${product.status === 'active' ? 'bg-[var(--primary)]/10 text-[var(--green-dark)] border border-[var(--primary)]/20' : 'bg-red-50 text-red-700 border border-red-100'}`}>
                      <div className={`w-1.5 h-1.5 rounded-full ${product.status === 'active' ? 'bg-[var(--primary)]' : 'bg-red-500'}`} />
                      {product.status}
                    </div>
                  </td>
                  <td className="py-4 px-4">
